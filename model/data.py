...
class MySQuAD():
    def __init__(self, args):
        ...
        dict_fields = {'id': ('id', self.RAW),
                       's_idx': ('s_idx', self.LABEL),
                       'e_idx': ('e_idx', self.LABEL),
                       'context': ('c_char', self.CHAR),
                       'question': ('q_char', self.CHAR)
                       }

        list_fields = [('id', self.RAW), ('s_idx', self.LABEL), ('e_idx', self.LABEL),
                       ('c_char', self.CHAR),('q_char', self.CHAR)]
        ...
        #cut too long context in the training set for efficiency.
        if args.context_threshold > 0:
            self.train.examples = [e for e in self.train.examples if len(e.c_char) <= args.context_threshold]
